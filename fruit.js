...
const isFoodHealthy = (food) =>{
  // Check if the food item is nutrient-dense
  if ((food['protein_g'] + food['fiber_g']) < (food['fat_total_g'] + food['carbohydrates_total_g'])) {
    return false;
  }

  // Check if the food item is low in added sugars and unhealthy fats
  if (food['fat_saturated_g'] > 5 || food['sugar_g'] > 25) {
    return false;
  }
